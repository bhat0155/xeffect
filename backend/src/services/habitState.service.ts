...
import type { HabitMeta, HabitState } from "../contracts/habitState";
...
export async function getHabitStateForUser(userId: string): Promise<HabitState>{
    ...
    const yesterdayUTC = getYesterdayUTCDate()
   ...
   if(habit){
    ...
    const recentCheckIns = await prisma.habitCheckin.findMany({
     ...
     checkinSet = checkinsToSet(recentCheckIns);
   }
    const computed= computeEngineState({todayUTC, yesterdayUTC, checkinSet, allDone: habit?.allDone ?? false})


   return {
        habit: habitMeta,
        todayUTC,
        checkedInToday: computed.checkedInToday,
